...
$(function(){
    ...
    $specialty.change(() => {
        ...
        if (specialtyId != '') {
            const url = base_url + `especialidades/${specialtyId}/medicos`;
...
function loadHours() {
    ...
    const url = base_url + `horario/horas?date=${selectedDate}&doctor_id=${doctorId}`;
...
function getRadioIntervaloHTML(intervalo){
    ...
    return `<div class="custom-control custom-radio mb-3">
            ...
            <label class="custom-control-label text-danger" for="interval${iRadio++}">${text}</label>
            </div>`;
}